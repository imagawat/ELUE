...
def my_positive_func(val):
    if val < 1:
        return torch.exp(val-1)
    return val

def clip_min(tensors, min_val):
...
def flatten(tensor):
    s_tuple = tensor.size()
    return tensor.view(-1, s_tuple[-1])

def flatten_all(tensors, contiguous=True):
    if contiguous:
        return [flatten(tensor.contiguous()) for tensor in tensors]
    return [flatten(tensor) for tensor in tensors]
