...
class HumanoidDirEnv(HumanoidEnv):
    ...
    def step(self, action):
        ...
        done = done or (self.time_steps >= self.max_episode_steps)
        return self._get_obs(), reward, done, dict(reward_linvel=lin_vel_cost,
                                                   reward_quadctrl=-quad_ctrl_cost,
                                                   reward_alive=alive_bonus,
                                                   reward_impact=-quad_impact_cost,
                                                   goal=[goal_direction[0], goal_direction[1], None], fingerXY=(pos_after - pos_before) / self.model.opt.timestep, objXY=[None, None]) #TODO: change name. "finger", "obj" is not appropriate.
