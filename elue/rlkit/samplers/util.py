...
def rollout(env, agent, max_path_length=np.inf, accum_context=True, update_belief=False, animated=False):
    """
    ...
    while path_length < max_path_length:
        ...
        #try:
            next_o, r, d, env_info = env.step(a)
        except Exception as e:# すべて拾う
            print("obs: ", o)
            print("action: ", a)
            print("policy: ",  [pa for pa in agent.policy.parameters()])
            print("encoder: ",  [pa for pa in agent.encoder.parameters()])
            print("integrator: ",  [pa for pa in agent.integrator.parameters()])
            print("decoder: ",  [pa for pa in agent.decoder.parameters()])
            print(e)
            import sys
            sys.exit()
        """
        ...
        if accum_context:
            ...
            if update_belief:
                agent.infer_posterior(incremental=True)
